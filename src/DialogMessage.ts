...
export namespace DialogMessage {
    ...
    export function getServerPortChangeErrorMessage(serverName: string, serverPort: string): string {
        return localize('tomcatExt.serverPortChangeError', 'Sever port of running server {0} was changed, which will cause server restart/stop errors, please change it back to {1} !', serverName, serverPort);
    }
